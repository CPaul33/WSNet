...
def get_img_norm_cfg(dataset_name, dataset_dir):
    if  dataset_name == 'SIRST':
        img_norm_cfg = dict(mean=111.97807312011719, std=27.62665367126465)
...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    if optimizer_name == 'Adam':
        ...
        optimizer  = torch.optim.SGD(net.parameters(), lr=optimizer_settings['lr'], momentum=0.9, weight_decay=0.0004)
    elif optimizer_name == 'Nesterov':
        optimizer  = torch.optim.SGD(net.parameters(), lr=optimizer_settings['lr'], nesterov=True, momentum=1, weight_decay=0)
