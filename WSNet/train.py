...
parser.add_argument("--weights_init", default=None, type=str, help="weights_init method: 'weights_init_xavier', 'weights_init_kaiming'")
...
def train():
    ...
    if opt.weights_init == 'weights_init_xavier':
        ...
        print('weights_init_xavier')
    if opt.weights_init == 'weights_init_kaiming':
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        for idx_iter, (img, gt_mask) in enumerate(tbar):
            ...
            loss.backward()
            optimizer.step()
            tbar.set_description('Training_Epoch: %d' % (idx_epoch + 1))
